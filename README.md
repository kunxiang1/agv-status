...
- **增量维护**：订阅线程检测某车 `podCode` 变化（= 取货或放货），调 `rcs_pods.apply_car_event`：
  取货→把该货架从表里摘掉（此刻画在车身上）；放货→按车当前坐标定落点，规则是
  **认可半径 0.5m 内取最近候选节点（路径点[类型16]不算候选——那是路网，货架不会停），
  且该点必须比「下一个不同位置」近出 0.25m 决断余量**；定不下来就不记（等「同步货架」校准），
  宁可暂时空白也不把货架画到隔壁格。落点若已被别的货架占着，顶掉旧记录（一格一货架）。
...
| `index.html` | 配置拉取 + 事件处理 + 弧长动画引擎（含挂起收敛）+ 渲染（含货架层/悬停）+ 交互 |
