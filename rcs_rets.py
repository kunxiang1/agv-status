# -*- coding: utf-8 -*-
"""地图背景（区域多边形+文字标注）与区域名字典（区域码→中文名）。

两份数据都是**随布局改版才动**的低频内容，因此**只在「⟳同步地图」时拉**：
  ...
  2) 区域名：POST {WEB_BASE}/rcms/web/areaType/getAreaAndSecByMapCode.action（form）
     {"mapCode":"BB"} -> rows:[{areaCode, areaName, ...}]（货架表 areaCode 的中文名）
  两者都是登录 Cookie 即可、无 IP 白名单（2026-09-13/14 实测四图全有数据）。
...
def pull_areas(op, map_code, timeout=12):
    ...
    行里还带 areaCat/isLock/liftCodes；地标码↔区域的成员表是另一个 action
    (/web/mapData/findListWithPages.action, start/limit 分页, 行含 dataName/areaCode/stgSecCode)。"""
...
def pull_all(maps=None, timeout=15):
    ...
    for mc in maps or C.MAP_CODES:
        try:
            ...
            out[mc] = {"polys": merged["polys"], "labels": merged["labels"], "areas": areas}
        except Exception as e:
    ...
    if not out:
        ...
        for qr, v in old.items():                 # 失败图也补 areas 键（前端取 RETS[qr].areas）
            v.setdefault("areas", {})
            v.setdefault("polys", [])
            v.setdefault("labels", [])
        out = old
    ...
    print("rcs_rets: 背景已更新 %s" % {k: (len(v.get("polys") or []), len(v.get("labels") or []),
                                              len(v.get("areas") or {})) for k, v in out.items()},
          flush=True)
    return out
